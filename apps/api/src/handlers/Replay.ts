...
import type { APIScoreSave } from '@owenii/types';
...
export class Replay extends null {
    public static async replayGame(
        game: GameDocument,
        save: APIScoreSave<GameEntity.Mode>
    ) {
        ...
        if (game.mode === GameEntity.Mode.HigherOrLower) {
            ...
            // TODO: Mark account as cheater if not valid
    ...
    private static async _isValidHigherOrLowerSave(
        game: GameDocument,
        save: APIScoreSave<GameEntity.Mode.HigherOrLower>
    ) {
        if (game.mode !== GameEntity.Mode.HigherOrLower)
            throw new Error(422, 'Invalid game type');
